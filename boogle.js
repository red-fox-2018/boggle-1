class Boggle {
  ...
  sendword() {
    ...
    let possible = ""
    let count = 0
    for (let i = 0; i < kamus.length; i++) { /** looping kata */
      // console.log(kamus[i]) 
      let tmp = []
      for (let k = 0; k < kamus[i].length - 1; k++) { /* looping huruf */
...
let fs = require('fs');
let kamus2 = fs.readFileSync('dictionary.txt').toString().split("\n")
